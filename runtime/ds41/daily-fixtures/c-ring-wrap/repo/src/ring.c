...
size_t ring_write(Ring*r,const unsigned char*src,size_t n){ if(n>r->cap-r->used)n=r->cap-r->used; size_t done=0; while(done<n){ size_t span=r->cap-r->head-1; if(span>n-done)span=n-done; if(span==0){r->head=0;continue;} memcpy(r->buf+r->head,src+done,span); r->head=(r->head+span)%r->cap; done+=span;} r->used+=done; return done; }
