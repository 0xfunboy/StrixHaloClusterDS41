...
ARM=${2:?serial|concurrent}
TIMING=${3:-0}
ROOT=/home/funboy/StrixHaloClusterDS41
DS4=/home/funboy/.local/share/haloclu-ds41/releases/ds4-speed-001-engram1
...
if [[ "$ARM" == serial ]]; then
    export DS4_V41_DISABLE_ENGRAM_CONCURRENT=1
elif [[ "$ARM" != concurrent ]]; then
    echo "invalid arm: $ARM" >&2
    exit 2
fi
if [[ "$TIMING" == 1 ]]; then export DS4_V41_ENGRAM_TIMING=1; fi

[[ -x "$DS4/ds4" && -x "$DS4/ds4-server" ]] || { echo DS4_SPEED_BINARY_GATE_FAIL >&2; exit 20; }
[[ -f "$MODEL" && $(stat -c %s "$MODEL") == 365713686528 ]] || { echo DS4_Q2_SIZE_GATE_FAIL >&2; exit 21; }
