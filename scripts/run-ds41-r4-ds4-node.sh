...
CTX=${DS41_R4_DS4_CTX:-16384}
COORD=${DS41_R4_DS4_COORD:-10.55.0.1}
TP_PORT=${DS41_R4_DS4_TP_PORT:-9911}
API_PORT=${DS41_R4_DS4_API_PORT:-8080}
...
case "$ROLE" in
 coordinator)
   ...
   exec "$DS4/ds4-server" --rocm -m "$MODEL" --ctx "$CTX" --tensor-parallel --role coordinator --listen "$COORD" "$TP_PORT" --transport tcp --batched-session 1 --host 127.0.0.1 --port "$API_PORT"
   ;;
 worker)
   exec "$DS4/ds4" --rocm -m "$MODEL" --ctx "$CTX" --tensor-parallel --role worker --coordinator "$COORD" "$TP_PORT" --transport tcp
   ;;
