...
o={"schema":"ds4-usable-release-001-lifecycle-v1","state":state,"detail":detail,"updated_unix":time.time(),
   "coordinator_unit":"ds4-usable-coordinator.service","worker_unit":"ds4-usable-worker.service",
   "api":"http://127.0.0.1:8080","tp":"10.55.0.1:9911","gate_timeout_ms":5000,"ctx":16384,
   "rollback":"k2-prefill-5bdfed6"}
t=p+".tmp";open(t,"w").write(json.dumps(o,indent=2)+"\n");os.replace(t,p)
PY
}
cleanup_restore(){
 systemctl --user stop ds4-usable-coordinator.service 2>/dev/null || true
 ssh -o IdentityAgent=none -o BatchMode=yes 02-evo-x3-tb 'systemctl --user stop ds4-usable-worker.service 2>/dev/null || true' || true
 sleep 2
...
write STARTING "DS4 usable pair launched; K2 OFF verified; waiting local API"
deadline=$((SECONDS+1200))
while ((SECONDS<deadline)); do
 lc=$(systemctl --user is-active ds4-usable-coordinator.service 2>/dev/null || true)
 rw=$(ssh -o IdentityAgent=none -o BatchMode=yes 02-evo-x3-tb 'systemctl --user is-active ds4-usable-worker.service 2>/dev/null || true' 2>/dev/null || true)
 ...
 if [[ "$code" == 200 ]]; then
   mv "$RAW/v1-models.tmp" "$RAW/v1-models.json"
   write READY "DS4 API HTTP200; both units active; K2 remains OFF"
   exit 0
 fi
 rm -f "$RAW/v1-models.tmp"
 sleep 5
done
