#!/usr/bin/env python3
"""Integrated DS41 mHC M=1 gate for the TileLang projection/RMS candidate.

Both arms keep the promoted coefficient/Sinkhorn fusion enabled.  The candidate
changes only the preceding M=1 projection/RMS path; M>1 must remain exact eager
fallback.
"""
...
OUT = ROOT / 'reports/DS41-Q2-001/perf/projection-rms/integrated-node01.json'
ATOL_FP32 = 5e-5
RTOL_FP32 = 5e-4
ATOL_POST = 2e-3
RTOL_POST = 2e-4
...
def metric(a, b):
    ...
    return {'max_abs':float(d.abs().max()),'mean_abs':float(d.abs().mean()),'rel_l2':float(d.norm())/max(rn,1e-30),'finite':bool(torch.isfinite(a).all() and torch.isfinite(b).all())}
...
def run_pair(label,residual,fn,scale,base,*,pre_mix=None,x=None):
    ...
    os.environ['DS41_MHC_PROJECTION_RMS']='0'
    ...
    for name,a,b in zip(('post_mix','comb_mix','layer_input','next_pre'),cand,ref,strict=True):
        if a.dtype == torch.float32:
            ...
            # The collapse itself is unchanged; for the tested cases the tiny
            # projection reduction difference must not perturb BF16 output.
            torch.testing.assert_close(a,b,atol=0,rtol=0)
        rows[name]=metric(a,b)
    probe=torch.randn((residual.shape[0],residual.shape[-1]),device='cuda',dtype=torch.bfloat16,generator=torch.Generator(device='cuda').manual_seed(918))
...
def random_regular(tokens):
    g=torch.Generator(device='cuda').manual_seed(28700+tokens)
...
def main():
    ...
    # M>1 candidate request must be exact fallback to the current path.
    for v in rows[-1]['metrics'].values():
        assert v['max_abs'] == 0.0
    from runtime.ds41.mhc_projection_rms import stats
    out={'status':'PASS','tolerances':{'fp32_atol':ATOL_FP32,'fp32_rtol':RTOL_FP32,'post_atol':ATOL_POST,'post_rtol':RTOL_POST},'cases':rows,'projection_stats':stats()}
    out_path=Path(os.environ.get('DS41_MHC_PROJECTION_GATE_OUT',str(OUT)))
    out_path.parent.mkdir(parents=True,exist_ok=True); out_path.write_text(json.dumps(out,indent=2)+'\n'); print(json.dumps(out,indent=2))
    os.environ.pop('DS41_MHC_PROJECTION_RMS',None)
