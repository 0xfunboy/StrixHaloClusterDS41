#!/usr/bin/env python3
"""Independent validator for DS41 same-load native-HIP + mHC coefficient/Sinkhorn A/B."""
...
def main():
    ...
    ab_token_compare = []
    for i in (1, 2):
        ba = r0[f"mhc-ab-baseline-{i}"]["token_ids"]
        ca = r0[f"mhc-ab-candidate-{i}"]["token_ids"]
    ...
    smoke = r0["mhc-ab-candidate-smoke"]
    smoke_pass = smoke.get("finished") and smoke.get("text", "").strip() == "323"
    coding = load_code_validator()(r0["mhc-ab-candidate-coding"].get("text", ""))
    try:
        obj = json.loads(r0["mhc-ab-candidate-json"].get("text", ""))
    ...
    reasoning = r0["mhc-ab-candidate-reasoning-high"]
    reasoning_answer = reasoning_final(reasoning.get("text", ""))
    reasoning_pass = reasoning.get("finished") and reasoning_answer == "10"

    stats0 = d0.get("mhc_stats") or {}
    stats1 = d1.get("mhc_stats") or {}
    execution_pass = (
        stats0.get("fused_calls", 0) > 0
        and stats1.get("fused_calls", 0) > 0
    ...
    ok = (
        d0.get("status") == "MHC_AB_COMPLETE"
        and d1.get("status") == "MHC_AB_COMPLETE"
